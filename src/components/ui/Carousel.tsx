...
export default function HardRockCarousel({ slides }: { slides: Slide[] }) {
  ...
  return (
    <section className="relative w-full">
      {/** 
       * 🔵 Hard Rock HERO SIZE 
       * h-[600px] = altura Hard Rock real
       */}
      <div ref={emblaRef} className="overflow-hidden h-[600px] w-full">
        <div className="flex h-full">
          {slides.map((s) => (
            <div key={s.id} className="relative flex-[0_0_100%] h-full w-full">
              {/* IMAGEN FULLSCREEN */}
              <img
                src={s.img}
              ...
              {/* 📍 TEXTO AL ESTILO HARD ROCK (izquierda) */}
