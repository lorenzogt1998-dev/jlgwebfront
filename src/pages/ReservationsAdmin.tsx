...
export default function ReservationsAdmin() {
  ...
  async function loadReservations() {
    ...
    try {
      ...
      const activeReservations = data.filter(
        (reservation: any) =>
          reservation.showDate && reservation.showDate.deleted === false
      );
  ...
  return (
    <div className="min-h-screen bg-gray-50 px-6 py-10 pt-28 flex justify-center">
          ...
          <table className="w-full border-collapse">
            <thead>
              <tr className="border-b border-gray-200">
                <th className="py-3 px-4 text-sm font-semibold text-[#243f4a]">
                  ID
                </th>
                <th className="py-3 px-4 text-sm font-semibold text-[#243f4a]">
                  Contact
                ...
                <th className="py-3 px-4 text-center text-sm font-semibold text-[#243f4a]">
                  Actions
                </th>
              </tr>
            </thead>
            ...
            <tbody>
              {reservations.map((r, i) => (
                ...
                >
                  ...
                  <td className="py-3 px-4 text-sm">
                    {formatDate(r.showDate.date)}
                  </td>
                  <td className="py-3 px-4 text-sm text-gray-700">
                    {r.showDate.tour.name}
                  </td>
      ...
      {editing && (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 px-4">
          <div className="bg-white shadow-lg rounded-3xl p-6 md:p-8 w-full max-w-lg border border-gray-100 max-h-[90vh] overflow-y-auto relative">
            <button
              onClick={() => {
                setEditing(null);
                showPopup(
                  "Edición cancelada",
                  "Se canceló la edición exitosamente."
                );
              }}
            ...
            <h2 className="text-xl md:text-2xl font-bold text-[#243f4a] mb-6 text-center">
              Editar Reserva
            </h2>

            <form
              onSubmit={async function handleSubmit(e: React.FormEvent) {
                e.preventDefault();
                ...
                try {
                  ...
                  await adminFetch(
                    `/api/show-dates/${editing.showDate.id}/status`,
                    {
                      method: "PATCH",
            ...
            >
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <input
                    type="text"
                    value={editing.organizationName}
                    onChange={(e) =>
                      setEditing({
                        ...editing,
                ...
                <div>
                  ...
                  <input
                    type="text"
                    value={editing.showDate.schoolName}
                    onChange={(e) =>
                      setEditing({
                        ...editing,
                        showDate: {
